...
public class PlayerHealth : NetworkBehaviour {
    ...
    /// </summary>
    [Command]
    void CmdDie(GameObject _owner, GameObject _playerObject, GameObject _killer, bool _headshot)
    {
        currentHealth = 0;      // If we died, set the health to 0.

        if (_owner != null)
            _owner.GetComponent<PlayerNetwork>().CmdPlayerDied(_owner, _playerObject, _killer, _headshot);  //Tell the owner that this playerObject died.
    }
    ...
    /// </summary>
    [ClientRpc]
    public void RpcTakeDamage(int _damage, GameObject _killer, bool _headshot)
    {
        if (hasAuthority)
        {
            ...
            if (currentHealth <= 0)         // If health drops below 0, we died
            {
                if (currentHealth < 0)
                    currentHealth = 0;
                //if(isClient)
                CmdDie(owner, gameObject, _killer, _headshot);  // Tell the server that we died, who our owner is and who killed us.
            }
        }
    }
}
